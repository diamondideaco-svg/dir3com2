...
export function createStayDemoSearch(search = searchLiteApiHotels, clock = Date.now) {
  const cache = new Map<string, { until: number; value: StayDemoResult }>();
  const pending = new Map<string, Promise<StayDemoResult>>();
  let lastStart = -Infinity;
  return async function run(query: StayDemoQuery, env: NodeJS.ProcessEnv = process.env): Promise<StayDemoResult> {
