...
- Closed independent review findings by rejecting direct, transitive, and ADMIN-option role-membership paths to telemetry privilege escalation, requiring a target-derived migration/ACL/RLS attestation before any live probe or telemetry write, isolating each direct-provider probe in a terminable worker, preserving one allowlisted failure record when persistence verification throws, and reducing terminal failures to stable non-secret codes. Windows filesystem ACL enforcement remains an operational prerequisite and is not claimed by the harness.
- Hardened the shared telemetry persistence and after-response scheduling log boundary so arbitrary writer/database exceptions and caller-supplied request identifiers are discarded; only fixed operation/failure categories and internally generated attempt identifiers reach terminal output. End-to-end certification coverage injects credential-bearing URLs, authorization text, raw codes/details, and hostile stacks and proves they never appear.
