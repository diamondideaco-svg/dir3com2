...
Historical Partner-only precheck: `20260906183519_customer_private_document_upload` was active locally but absent from the ledger inspected for that release. It was subsequently applied under separate authorization as `20260909171237_customer_private_document_upload`; its byte-identical repository rename is documented in [Customer Documents parity closure](customer-documents-production-parity.md). The earlier Partner evidence is not authorization for a blanket Production `db push` or Customer Documents re-execution.
