...
CREATE POLICY IF NOT EXISTS "Public read partners" ON partners FOR SELECT USING (status IN ('active','pending'));
CREATE POLICY IF NOT EXISTS "Service role full access partners" ON partners FOR ALL USING (auth.role() = 'service_role') WITH CHECK (auth.role() = 'service_role');
CREATE POLICY IF NOT EXISTS "Service role full access partner_documents" ON partner_documents FOR ALL USING (auth.role() = 'service_role') WITH CHECK (auth.role() = 'service_role');
CREATE POLICY IF NOT EXISTS "Service role full access partner_services" ON partner_services FOR ALL USING (auth.role() = 'service_role') WITH CHECK (auth.role() = 'service_role');
CREATE POLICY IF NOT EXISTS "Service role full access partner_coverage" ON partner_coverage FOR ALL USING (auth.role() = 'service_role') WITH CHECK (auth.role() = 'service_role');
CREATE POLICY IF NOT EXISTS "Service role full access partner_performance" ON partner_performance FOR ALL USING (auth.role() = 'service_role') WITH CHECK (auth.role() = 'service_role');
