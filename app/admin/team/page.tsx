...
import { AdminLocalizedInput, AdminRetryButton, AdminStatusText, AdminText, AdminPermissionText, AdminSubmitButton } from '@/components/admin/AdminLocale';
  ...
  if (!await isCeoActor(supabase, user)) {
    return (
      <main className="mx-auto max-w-4xl px-4 py-10 lg:px-8">
        <div className="rounded-3xl border border-red-400/30 bg-red-500/10 p-6 text-red-800">
  ...
  if (state.status !== 'ready') {
    return (
      <main className="mx-auto w-full min-w-0 max-w-7xl px-4 py-8 lg:px-8">
        ...
        <section role="status" className="mt-6 rounded-3xl border border-slate-200 bg-white p-6">
  ...
  return (
    <main className="mx-auto w-full max-w-7xl px-4 py-8 lg:px-8">
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-6">
        ...
        <form action={upsertTeamAccessGrantAction} className="mt-6 grid gap-4 md:grid-cols-2">
          <label className="grid gap-2 text-sm">
            ...
            <input name="email" type="email" required className="rounded-xl border border-slate-300 bg-white px-3 py-2 text-slate-900" />
          </label>
          <label className="grid gap-2 text-sm">
            <span><AdminText ar="المسمى الوظيفي" en="Job title" /></span>
            <AdminLocalizedInput name="jobTitle" required ar="مدير مصر" en="Egypt Manager" className="min-w-0 rounded-xl border border-slate-300 bg-white px-3 py-2 text-slate-900" />
          </label>
          <label className="grid gap-2 text-sm">
            <span><AdminText ar="مستوى الصلاحيات" en="Access level" /></span>
            <select name="accessLevel" defaultValue="scoped_staff" className="rounded-xl border border-slate-300 bg-white px-3 py-2 text-slate-900">
          ...
          <label className="grid gap-2 text-sm">
            ...
            <input name="countryScope" placeholder="EG, QA" className="rounded-xl border border-slate-300 bg-white px-3 py-2 text-slate-900" />
          </label>
          <fieldset className="md:col-span-2">
            ...
            <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
              {TEAM_PERMISSIONS.filter((permission) => permission !== 'admin:full').map((permission) => (
                <label key={permission} className="flex items-center gap-2 rounded-xl border border-slate-200 p-3 text-sm">
                  <input type="checkbox" name="permissions" value={permission} />
                  <span><AdminPermissionText value={permission} /></span>
                </label>
              ))}
            </div>
          </fieldset>
          <div className="md:col-span-2">
            <AdminSubmitButton ar="حفظ صلاحيات الموظف" en="Save employee access" confirmAr="تأكيد حفظ صلاحيات الموظف وإرسال دعوة إذا لزم الأمر؟" confirmEn="Save employee access and send an invitation if needed?" className="rounded-full bg-[#D4AF37] px-5 py-2.5 font-semibold text-[#0D1B2A]" />
          </div>
        </form>
      </section>
      ...
      <section className="mt-8 space-y-4">
        {grants.map((grant) => (
          <article key={grant.id} className="rounded-3xl border border-slate-200 bg-white p-5">
            <div className="flex flex-wrap items-start justify-between gap-4">
              ...
              <form action={setTeamAccessStatusAction}>
                ...
                <AdminSubmitButton disabled={isCeoUserId(grant.invited_user_id)} ar={grant.status === 'active' ? 'إيقاف' : 'تفعيل'} en={grant.status === 'active' ? 'Deactivate' : 'Activate'} confirmAr="تأكيد تغيير حالة وصول هذا الموظف؟" confirmEn="Confirm changing this employee’s access status?" className="rounded-full border border-slate-300 px-4 py-2 text-sm disabled:opacity-40" />
              </form>
            </div>
            <div className="mt-4 grid gap-2 text-sm text-slate-300 md:grid-cols-2">
              <p><AdminText ar="البلدان" en="Countries" />: {(grant.country_scope ?? []).join(', ') || '—'}</p>
              <p className="break-words"><AdminText ar="الصلاحيات" en="Permissions" />: {grant.permissions?.length ? grant.permissions.map((permission, index) => <span key={permission}>{index > 0 ? ' · ' : ''}<AdminPermissionText value={permission} /></span>) : '—'}</p>
            </div>
          </article>
        ))}
