      ...
      <div className="mx-auto max-w-5xl">
        <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <h1 className="mt-2 text-3xl font-semibold"><AdminText ar={product.name_ar || product.name_en} en={product.name_en || product.name_ar} /></h1>
          </div>
          <div className="flex gap-2">
            {(scope.mode === 'global' || hasPermission(scope.grant, 'products:write')) && <Link href={`/admin/products/${encodeURIComponent(id)}`} className="rounded-full border border-[color:var(--color-border)] px-4 py-2 text-sm font-semibold"><AdminText ar="تعديل" en="Edit" /></Link>}
        ...
        <section className="overflow-hidden rounded-[2rem] border border-[#D4AF37]/25 bg-white shadow-sm">
          ...
          <div className="grid gap-6 p-6 lg:grid-cols-[1fr_0.8fr]">
            <div>
              ...
              <h2 className="mt-4 text-2xl font-semibold text-[#0D1B2A]"><AdminText ar={product.name_ar} en={product.name_en} /></h2>
              <p className="mt-2 text-sm leading-7 text-[#64748B]"><AdminText ar={product.description_ar || '—'} en={product.description_en || '—'} /></p>
            </div>
            ...
            <dl className="grid gap-3 rounded-2xl bg-[#FAF8F4] p-5 text-sm">
              ...
              <div className="flex justify-between gap-4"><dt><AdminText ar="التنفيذ" en="Fulfilment" /></dt><dd><AdminStatusText value={product.fulfilment_state} /></dd></div>
              <div className="flex justify-between gap-4"><dt><AdminText ar="المعاملة" en="Transaction" /></dt><dd><AdminStatusText value={product.transaction_method} /></dd></div>
              <div className="flex justify-between gap-4"><dt><AdminText ar="التوريد" en="Supply" /></dt><dd><AdminStatusText value={product.supply_type} /></dd></div>
