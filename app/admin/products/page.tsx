      ...
      <div className="mx-auto max-w-[1500px]">
        <div className="mb-8 flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-sm text-[#64748B]"><AdminText ar="إنشاء مسودة ← معاينة ← نشر ← إلغاء نشر أو أرشفة. كل خطوة واضحة ومدقّقة." en="Create draft → Preview → Publish → Unpublish or Archive. Each lifecycle step is explicit and audited." /></p>
          </div>
          {isGlobal ? (
        ...
        <form method="get" className="mb-6 grid gap-3 rounded-[1.5rem] border border-[color:var(--color-border)] bg-white p-4 sm:grid-cols-2 xl:grid-cols-[1.5fr_repeat(4,minmax(130px,1fr))_auto]">
          <label className="grid gap-1 text-xs font-semibold text-[#64748B]"><AdminText ar="البحث" en="Search" /><input name="q" defaultValue={params.q || ''} placeholder="Name / slug / city" className="min-h-11 rounded-xl border border-[color:var(--color-border)] px-3 text-sm text-[#334155]" /></label>
          <label className="grid gap-1 text-xs font-semibold text-[#64748B]"><AdminText ar="الحالة" en="Status" /><select name="status" defaultValue={params.status || 'all'} className="min-h-11 rounded-xl border border-[color:var(--color-border)] bg-white px-3 text-sm text-[#334155]"><option value="all">All</option><option value="draft">Draft</option><option value="published">Published</option></select></label>
          <label className="grid gap-1 text-xs font-semibold text-[#64748B]"><AdminText ar="العائلة" en="Family" /><select name="family" defaultValue={params.family || 'all'} className="min-h-11 rounded-xl border border-[color:var(--color-border)] bg-white px-3 text-sm text-[#334155]"><option value="all">All</option><option value="drive">Drive</option><option value="stay">Stay</option><option value="fly">Fly</option><option value="concierge">Concierge</option><option value="vip">VIP</option></select></label>
          <label className="grid gap-1 text-xs font-semibold text-[#64748B]"><AdminText ar="المدينة" en="City" /><select name="city" defaultValue={params.city || 'all'} className="min-h-11 rounded-xl border border-[color:var(--color-border)] bg-white px-3 text-sm text-[#334155]"><option value="all">All</option>{cities.map((cityName) => <option key={cityName} value={cityName.toLowerCase()}>{cityName}</option>)}</select></label>
          <label className="grid gap-1 text-xs font-semibold text-[#64748B]"><AdminText ar="الشريك" en="Partner" /><input name="partner" defaultValue={params.partner === 'all' ? '' : params.partner || ''} placeholder="Partner ID" className="min-h-11 rounded-xl border border-[color:var(--color-border)] px-3 text-sm text-[#334155]" /></label>
          <div className="flex items-end gap-2"><button type="submit" className="min-h-11 rounded-full bg-[#0D1B2A] px-5 text-sm font-semibold text-white"><AdminText ar="تطبيق" en="Apply" /></button><Link href="/admin/products" className="inline-flex min-h-11 items-center rounded-full border border-[color:var(--color-border)] px-4 text-sm font-semibold"><AdminText ar="مسح" en="Clear" /></Link></div>
        </form>
