...
import { DRIVE_CURRENCIES, DRIVE_OFFERS, vehicleFor, vehicleTitle, vehicleClassLabel, type VehicleClass, type DriveOffer, type VehicleMaster } from '@/lib/drive/catalog';
  ...
  return <section className={styles.page} dir={direction}>
    ...
    {searched && !loading && !error && <>
      ...
      <div className={styles.grid}><aside className={styles.filters}>
        ...
        {visible.map(offer=><article className={styles.card} key={offer.id}><Image className={styles.cardImage} src={offer.vehicle.image} alt={vehicleTitle(offer.vehicle,language)} width={500} height={300}/><div className={styles.cardBody}><h2>{vehicleTitle(offer.vehicle,language)}</h2><span className={styles.badge}>{ar?'طلب للتأكيد':'Request to confirm'}</span><p>{vehicleClassLabel(offer.vehicle.vehicleClass,language)}</p><DriveInclusions ar={ar}/><DrivePrice offer={offer} ar={ar}/>{offer.price.baseAmount===null?<p>{ar?'لم يقدّم المورد سعر انتقال المطار لهذا الطراز.':'The supplier has not provided an airport rate for this model.'}</p>:<Link className={styles.button} href={`/marketplace/drive/${offer.id}?${initialSearch}`}>{ar?'عرض التفاصيل':'View deal'}</Link>}</div></article>)}
      </section></div></>}
