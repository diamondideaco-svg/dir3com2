...
export default function Bookings({ bookings, requests, failed }: { bookings: BookingEngineRecord[]; requests: CustomerMarketplaceRequest[]; failed: boolean }) {
  ...
  return <>
    ...
    })}</div> : <div className={styles.empty}><p>{ar ? 'لا توجد حجوزات تطابق هذا العرض.' : 'No bookings match this view.'}</p><Link href="/marketplace" className={styles.primary}>{ar ? 'استكشف الخدمات' : 'Explore services'}</Link></div>}
