...
export default function ProductLifecycleControls({ id, slug, status, lifecycleVersion, canWrite = false }: Props) {
  ...
  const confirmAction = (messageAr: string, messageEn: string) => (event: React.FormEvent<HTMLFormElement>) => {
    if (submittingRef.current || !window.confirm(ar ? messageAr : messageEn)) {
      event.preventDefault();
      return;
    }

    // A redirect/revalidation replaces this component after a successful action.
    // Until then, lock every sibling lifecycle form to the same rendered version.
    ...
    <div className="flex flex-wrap items-center gap-2" aria-busy={submitting}>
      ...
      {canWrite && status === 'draft' ? (
        <form action={publishProductAction} onSubmit={confirmAction('نشر هذا المنتج في السوق؟', 'Publish this product to the Marketplace?')}>
          ...
          <button disabled={!active || submitting} className="min-h-10 rounded-full bg-[#D4AF37] px-3 py-2 text-xs font-bold text-[#0D1B2A] disabled:cursor-not-allowed disabled:opacity-45">
            {ar ? 'نشر' : 'Publish'}
          </button>
        </form>
