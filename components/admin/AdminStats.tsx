...
export default function AdminStats({ stats }: { stats: { total: number; pending: number; confirmed: number; completed: number; revenue: number | null } }) {
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '16px' }}>
      ...
      <div style={{ background: '#FFFFFF', border: '1px solid rgba(212, 175, 55, 0.1)', borderRadius: '16px', padding: '18px' }}>
        ...
        <div style={{ color: '#8B6516', fontSize: '1.6rem', fontWeight: 'bold', marginTop: '6px' }}>{stats.revenue === null ? <AdminText ar="غير متاح" en="Unavailable" /> : <AdminCurrency value={stats.revenue} />}</div>
      </div>
    </div>
  );
}
