...
const toneMap: Record<string, string> = {
  ...
  silver: 'bg-slate-400/15 text-slate-200',
  basic: 'bg-stone-500/15 text-stone-300',
};

export default function ShieldLevelBadge({ level }: ShieldLevelBadgeProps) {
  return (
    <span className={`inline-flex rounded-full px-3 py-1 text-xs font-semibold ${toneMap[level] || 'bg-slate-500/15 text-slate-300'}`}>
      {level}
    </span>
  );
}
