...
INSERT INTO public.products (name_ar, name_en, slug, city, base_price, currency, status, verified) VALUES ('???????? ???????????? ??????????', 'Local QA Test Service', 'local-qa-test-service', 'Local QA', 100, 'SAR', 'active', true);
