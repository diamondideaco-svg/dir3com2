...
function parseSupabaseRef(url) {
  try {
    return String(new URL(url).hostname.split('.')[0] || '').trim().toLowerCase();
  } catch {
    return null;
  }
}
...
function assertSafeEnvironment() {
  ...
  if (nodeEnv === 'production') {
    fail('Blocked: production runtime detected.');
  }

  if (TARGET_ENV !== 'staging') {
    fail('Blocked: SANDBOX_TARGET_ENV must be staging for sandbox scripts.');
  }

  if (APP_ENV === 'production') {
    fail('Blocked: production app environment detected.');
  }

  const refFromUrl = parseSupabaseRef(SUPABASE_URL);
  if (!refFromUrl) {
    fail('Blocked: SUPABASE_URL is not verifiable.');
  }

  const explicitRef = String(process.env.SUPABASE_PROJECT_REF || '').trim().toLowerCase();
  if (explicitRef && explicitRef !== refFromUrl) {
    fail('Blocked: SUPABASE_PROJECT_REF mismatch.');
  }

  const effectiveRef = explicitRef || refFromUrl;
  if (effectiveRef !== STAGING_PROJECT_REF) {
    fail('Blocked: project ref is not in staging allowlist.');
  }
