...
async function main() {
  ...
  const { callDeepSeekWebSearch } = await import('@/lib/ai2/runtime/deepseek-web');
  ...
  for (const check of checks) {
    const result = await callDeepSeekWebSearch({ message: check.message, language: check.language, prompt: 'Return only the exact requested phrase.', apiKey, model });
    console.log(`${check.label}: ${result.ok && result.answer.trim() === check.expected ? 'PASS' : `FAIL (${result.status ?? result.errorCategory ?? 'unknown'})`}`);
